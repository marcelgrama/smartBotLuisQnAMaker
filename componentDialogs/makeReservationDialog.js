...
const { DialogSet, DialogTurnStatus, ConfirmPrompt, ChoicePrompt, DateTimePrompt, NumberPrompt, TextPrompt } = require('botbuilder-dialogs');
...
class MakeReservationDialog extends ComponentDialog {
  ...
  async firstStep(step) {
    ...
    //running a prompt here means the next waterfallStep will be run when the users response is receiveed.
  ...
  async summaryStep(step) {
    ...
    if (step.result === true) {
      // business 
      await step.context.sendActivity('Reservation successfully made. Your reservation id is: 123456');
      endDialog = true;
      return await step.endDialog();
    }
  ...
  async noOfParticipantsValidator(promptContext) {
    return promptContext.recognized.succedded && promptContext.recognized.value > 1 && promptContext.recognized.value < 150;
  }
